...
renderCUDA(
	...
	const float* __restrict__ rayimage
	)
{
	...
	if (inside)
	{
		...
		n_contrib[pix_id] = last_contributor;
            ...
            // for (int ch = 0; ch < CHANNELS; ch++)
		    // 	out_color[ch * H * W + pix_id] = C[ch] + T * bg_color[ch]; add back if bg_color is 1,1,1
           	// printf("a: %f, b: %f, c: %f\n", bg_color, bg_color, bg_color);


			int allpix = H * W; // num pixel per image. macro ?
			int ch0 =  pix_id ;
			int ch1 =  allpix *1 + pix_id ; // replace * with +
			int ch2 =  allpix *2 + pix_id ;
			int ch3 =  allpix *3 + pix_id ;
			int ch4 =  allpix *4 + pix_id ;
			int ch5 =  allpix *5 + pix_id ;

			// float spec0 = C[3]; //
			// float spec1 = C[4];
			// float spec2 = C[5];

			// float time0 = C[6]; //time feature1
			// float time1 = C[7]; //time feature2
			// float time2 = C[8]; //time feature3
			...
			out_color[ch2] =  C[2]  +  mlpresult2[2];
			//printf("a: %f, b: %f, c: %f\n", mlpresult2[0], mlpresult2[1], mlpresult2[2]);
	}
}
...
{  
	...
	prepreprocessCUDA<NUM_CHANNELS> << <(P + 255) / 256, 256 >> > (
		...
		opacities,
		opacitiesdummy);
	//prepreprocessCUDA<NUM_CHANNELS> << <(P + 255) / 256, 256 >> > (P);
