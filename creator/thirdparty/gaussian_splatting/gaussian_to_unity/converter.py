...
def gaussian_timestep_to_unity(means3d: torch.tensor, 
    ...
    if ((args.dynamic_others) or idx == 0):
    ...
    if (args.dynamic_color):
        create_colors_asset(means3d.shape[0], col_to_save, color_format=args.col_format, basepath=basepath, i= idx)
...
def gaussian_static_data_to_unity(splat_count: int,
    ...
    if shs == None:
        shs = torch.zeros((splat_count, 16, 3))
