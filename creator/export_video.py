...
def create_frame_image(pc, timestamp, xyz_min=-1, xyz_max=1):
    ...
    colors = (colors - min_thresholds["_colors"]) / (max_thresholds["_colors"] - min_thresholds["_colors"]) * 65535
    colors = colors.astype(np.uint16)

    # rotations
    rotations = to_numpy_img(rotations_final)
    rotations = (rotations - min_thresholds["_rotation"]) / (max_thresholds["_rotation"] - min_thresholds["_rotation"]) * 65535
    rotations = rotations.astype(np.uint16)

    # opacities
    opacities = to_numpy_img(opacities_final)
    opacities = (opacities - min_thresholds["_opacity"]) / (max_thresholds["_opacity"] - min_thresholds["_opacity"]) * 65535
    opacities = opacities.astype(np.uint16)

    # scales
    scales = to_numpy_img(pc._scaling)
    scales = (scales - min_thresholds["_scaling"]) / (max_thresholds["_scaling"] - min_thresholds["_scaling"]) * 65535
...
if __name__ == "__main__":
    ...
    for idx, section in enumerate(sorted_sections):
        ...
        with torch.no_grad():
            ...
            t = [frame/args.section_size for frame in range(time_range[0], time_range[1])]
    ...
    (
        ...
        .output(os.path.join(outpath ,"scene.mp4"), pix_fmt="yuva444p10le", 
